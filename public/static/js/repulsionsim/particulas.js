...
function Particle(x, y, m, q, s, id) {
  ...
  this.update = function () {
    ...
    this.pos.x = this.pos.x < 0 ? 0 : this.pos.x > C.width ? C.width : this.pos.x;
    this.spd.y *= this.pos.y < 0 || this.pos.y > C.height ? -1 : 1;
    this.pos.y = this.pos.y < 0 ? 0 : this.pos.y > C.height ? C.height : this.pos.y;
    //Actualiza velocidad
    ...
    particles.forEach((p) => {
      ...
      if (d != 0) {
        ...
        this.acc.x += -acMag * Math.cos(Math.PI + Math.atan2(this.pos.y - p.pos.y, this.pos.x - p.pos.x));
        this.acc.y += -acMag * Math.sin(Math.PI + Math.atan2(this.pos.y - p.pos.y, this.pos.x - p.pos.x));
      }
    });
    if (this.isDead) {
...
function draw() {
  ...
  if (showFl) {
    ...
    for (let i = 0; i < rows; i++) {
      for (let j = 0; j < cols; j++) {
        ...
        ctx.fillStyle = flhere < 0 ? "hsl(" + accs[j + i * cols] + ", 100%, 50%)" : "hsl(" + accs[j + i * cols] + ", 100%, 50%)";
        ctx.beginPath();
        ctx.arc(j * flSize, i * flSize + Math.sqrt(accs[j + i * cols]) * Math.sign(accs[j + i * cols]) * 2, 5, 0, Math.PI * 2);
        ctx.fill();
  ...
  if (pause) {
    ctx.font = '2svh Arial';
    ctx.fillStyle = 'black';
    ctx.fillText("El campo aquí es: " + getField(cursor.x, cursor.y), cursor.x, cursor.y);
  }
  updateData();
